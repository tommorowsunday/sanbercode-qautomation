...
describe("Regist Toko in Casier Aja", () => {
  const response = request("https://kasirdemo.vercel.app/").post("/registration").send(login);
...
describe("Regist User", () => {
  const response = request("https://kasirdemo.vercel.app/").post("/authentications").send(auth);
  it("Geth Auth", async () => {
    console.log((await response).status);
    console.log((await response).body);
  ...
  it("Create User", async () => {
    const response = request("https://kasirdemo.vercel.app/").post("/users").send(user).set("Authorization", `Bearer ${token}`);
  ...
  it("Get User Detail", async () => {
    const response = request("https://kasirdemo.vercel.app/").get(`/users/${UserID}`).set("Authorization", `Bearer ${token}`);
    console.log("status adalah = " + (await response).status);
    console.log("Response Body = ");
    console.log((await response).body);
  ...
  it("Update Data", async () => {
    const response = request("https://kasirdemo.vercel.app/").put(`/users/${UserID}`).send(update).set("Authorization", `Bearer ${token}`);
    console.log("status adalah = " + (await response).status);
    console.log("Response Body = ");
    console.log((await response).body);
    expect((await response).status).to.equal(200);
  ...
  it("Delete Data", async () => {
    const response = await request("https://kasirdemo.vercel.app/").del(`/users/${UserID}`).set("Authorization", `Bearer ${token}`);
    console.log("status adalah = " + (await response).status);
    console.log("Response Body = ");
    console.log((await response).body);
    expect((await response).status).to.equal(200);
